...
The latest completed full offline reactor result is **487 passing Maven tests across seven modules** for Phase 6B. The current Foundry contract suite remains at **nine tests** from Phase 5D and was not rerun because Phase 6B changes no Solidity behavior. The default business API remains `POST /v1/token-operations/mints`, `POST /v1/token-operations/burns`, participant-scoped `GET /v1/token-operations/{operationId}`, `POST /v1/transfers`, and participant-scoped `GET /v1/transfers/{transferId}`. The combined local profiles additionally expose participant-scoped acquisition/redemption POST/GET resources under `/v1/usdzelle`; these are synthetic local workflow commands, not production banking endpoints. Durable Phase 3C transfer acceptance remains independent and does not invoke them.

## Designed, Not Executable

- Phase 5C transfer and Phase 5D redemption-custody/burn remain internal local-Anvil primitives, not executed Phase 3C parent effects. Phase 6B composes only the configured local user-held acquisition/redemption path; it is not a production balance product, bank integration, reserve, or custody system.
...
```text
Implemented now: domain + durable API/worker + transfer parent + signing + local Ethereum effects + configured local custody + synthetic finance + user-held workflows
Next: settlement-only orchestration (Phase 6C)
Then: reproducible local environment for both Ethereum demos
...
The `local-demo` profile composes the immutable wallet registry plus four synthetic bank identities, the two configured user accounts, and the internal reserve/liability services. Its separate local bank contract remains at `/local/v1/mock-banks/openapi.yaml`; withdrawals, deposits, account reads, and operation inquiry require distinct `local-bank:debit`, `local-bank:credit`, and `local-bank:read` authorities. Combined with `local-ethereum`, it also exposes the Phase 6B acquisition/redemption contract at `/openapi/usdzelle-workflows-v1.yaml` with distinct `usdzelle:acquire`, `usdzelle:redeem`, and `usdzelle:read` authorities. Only the configured `USER_1` participant mapping is orchestrated. The fixtures and ledger are synthetic POC evidence—not real funds, reserve assets, audited statements, attestation, or legal/customer/accounting finality.
...
`local-demo` accepts only chain ID `31337`, derives and validates every address, fails when a required identity is incomplete, and is mutually exclusive with `local-signer`. Alone it creates no RPC client or chain effect; it enables only the profile-isolated synthetic-bank API and internal accounting services described above. Combined with `local-ethereum`, it composes the internal Phase 5C/5D handlers and Phase 6B's acquisition/redemption parents. Parent acceptance persists exact participant, bank, asset/unit, network, contract, wallet, and policy versions before returning 202; delivery advances one durable boundary at a time, and redemption cannot dispatch burn before the one-time payout and payout accounting steps complete. These environment variables are acceptable only for this local reference profile; production custody requires workload identity/secret management plus HSM, MPC, or custody infrastructure behind the same signer port.

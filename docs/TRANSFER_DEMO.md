...
Phase 7D must add redemption custody and ADMIN burn parity. Phase 7E may then compose the user-held and settlement-only parents from the separately verified native primitives, and Phase 7F may package both full product demonstrations. Those slices must preserve native accounts, owners, instructions, signatures, slots, commitment, expiry, and SPL Token evidence rather than relabeling Ethereum receipts. ADRs 0003 and 0010 retain classic SPL Token first, no Neon baseline, exact atomic conversion outside the CLI decimal parser, and Rust/Anchor only if existing programs cannot express required behavior.

The detailed dependency sequence and exit gates are in [`docs/IMPLEMENTATION.md`](IMPLEMENTATION.md). Phase 7D native Solana redemption custody and burn remains next and requires separate authorization.

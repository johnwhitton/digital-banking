...
Phase 7A now proves the shared local semantic substrate with pinned native Agave/SPL tooling: classic checked instructions, canonical associated accounts, exact 10,000-base-unit movement at two decimals, classified authority and valid-second-mint/account rejection, actual transaction signatures/recent blockhashes/slots, validity-window observations, finalized commitment, and zero end state. It does not execute either durable Java product path.

Solana follows through the same provider-neutral business contracts after that gate. Its future adapter must preserve native fee payer/authority, accounts, instructions, recent blockhash or durable nonce, signature, slot, commitment, expiry, and SPL Token evidence. ADRs 0003 and 0010 require a bounded Java-client gate, classic SPL Token first, no Neon baseline, exact atomic conversion outside the CLI decimal parser, and Rust/Anchor only if existing programs cannot express required behavior.

The detailed dependency sequence and exit gates are in [`docs/IMPLEMENTATION.md`](IMPLEMENTATION.md). Phase 7B Java-client compatibility and mint parity remain next and require separate authorization.

...
Phase 7C must add native transfer, redemption custody, and burn parity before Phase 7D may compose either full product demonstration. Those slices must preserve native accounts, owners, instructions, signatures, slots, commitment, expiry, and SPL Token evidence rather than relabeling Ethereum receipts. ADRs 0003 and 0010 retain classic SPL Token first, no Neon baseline, exact atomic conversion outside the CLI decimal parser, and Rust/Anchor only if existing programs cannot express required behavior.

The detailed dependency sequence and exit gates are in [`docs/IMPLEMENTATION.md`](IMPLEMENTATION.md). Phase 7C native Solana transfer and burn parity remains next and requires separate authorization.

...
The current repository contains documentation, plain-Java exact operation, transfer, signing, synthetic-bank, accounting, user-held workflow, and settlement-orchestration invariants, framework-free application use cases/ports, PostgreSQL, local-signer, Ethereum-Web3j, and Solana-Sava adapters, a minimal local reference token, and a Spring Boot control plane. Phases 3A-3C provide durable token-operation/transfer acceptance, delivery/recovery, and the historical first internal transfer preparation. Phases 4A-4B provide durable signing authority and session-only local signing. Phases 5A-5D provide bounded local-Anvil mint, configured custody, user-wallet transfer, redemption custody, and ADMIN burn primitives. Phase 6A adds exact durable synthetic withdrawals/deposits/inquiry plus a closed append-only reserve/liability ledger and supply/custody reconciliation under `local-demo`. Phase 6B composes those primitives under `local-demo,local-ethereum` into separate participant-scoped acquisition and redemption parents with immutable server-resolved context, payout-before-burn, and reconciliation. Phase 6C adds a V10 companion to the existing transfer resource for one registered sender-acquisition, exact custody-transfer, recipient-`AUTO_REDEEM`, and reconciliation route. Phase 6D packages both paths in a digest-pinned, loopback-only Compose environment with local bearer identities, a bounded read-only operator projection, deterministic contract bootstrap, exact API-driven assertions, durable PostgreSQL/Anvil restart recovery, and scoped teardown. Phase 7A separately adds pinned repository-local native Agave/SPL tooling and a loopback semantic gate for classic checked mint, transfer, redemption custody, and burn with exact native evidence; Phases 7B-7D add bounded local Java mint, wallet-transfer, redemption-custody, and ADMIN-burn execution. Phase 7E lets the same Phase 6B/6C parents select those Solana effects under `local-demo,local-solana`, with chain-owned queue routing, immutable server-resolved wallet/signing context, exact finalized evidence, and unchanged payout-before-burn and reconciliation. Phase 7F packages both existing product paths with host-native Agave/Java, the approved cached PostgreSQL image, generated local bearer identities, exact API assertions, restart recovery, and scoped teardown. There is still no arbitrary institutional route, automatic compensation, production accounting/reserve, public wallet-transfer API, or production custody/network. The default configuration still has no identity/signer provider, bank/accounting/workflow bean, or enabled worker. The [two local demonstrations](TRANSFER_DEMO.md) record this boundary and the remaining work.
...
A user-facing on-ramp request cannot invoke unrestricted mint authority. Phase 6B accepts only exact amount/currency, a configured participant-owned synthetic bank reference, and optional local Ethereum selection; it resolves the user/ADMIN wallets, contract, asset/unit, signing, and policy context server-side and persists it before any effect. Mint and burn remain privileged child effects authorized by the parent workflow, reserve, policy, approval, and evidence gates.
...
| `ADMIN_REDEMPTION` | Receives tokens surrendered for redemption before burn | Separate redemption-purpose authority even when the local POC aliases it to ADMIN | Phase 6B's local redemption parent composes custody, payout-before-burn, burn, and reconciliation for the configured participant |
...
Controlled inventory starts at zero and is never an unexplained balancing plug. The accounting model can represent payout and burn in either order after confirmed custody; Phase 6B selects payout-before-burn. Its parent cannot dispatch payout before custody accounting or burn before payout accounting, and one retained payout identity prevents a crash or burn delay from paying twice. Missing, stale, non-final, removed, non-canonical, mismatched, or already-consumed evidence cannot post. Reconciliation records `RECONCILED`, reserve-ledger mismatch, chain-supply mismatch, incomplete evidence, or unsupported/stale observation and never mutates balances to force agreement.
...
The current reactor contains `domain`, `application`, `adapters/persistence-postgres`, `adapters/signer-local`, `adapters/ethereum-web3j`, `adapters/solana-sava`, and `control-plane`; the Foundry project is under `contracts/evm/`. Its concrete direction is `domain <- application <- adapters <- control-plane`; each adapter implements application-owned ports, and no framework, database, cryptographic-provider, Web3j, Sava, JSON-RPC, or native transaction type enters domain/application signatures. The control plane composes the adapters but does not directly declare or import Web3j or Sava. ADRs 0002 and 0003 select isolated Ethereum and native-Solana adapter boundaries; no module may depend on another chain adapter.
...
Phase 7A makes the pre-adapter semantics executable without promoting CLI or RPC output to business truth. The local gate records each successful transaction's actual signature, recent blockhash, slot, finalized commitment, checked instruction, and exact atomic quantity. It uses a second valid classic-SPL mint to classify the negative mint/account boundary and records only sanitized rejection outcomes after proving supplies and balances are unchanged. Because the pinned online SPL CLI selects its own fresh blockhash, the gate records that actual identity and live validity separately from the latest pre-submission blockhash/`lastValidBlockHeight` window. Phases 7B-7C instead persist the exact selected blockhash and last-valid height before isolated signing. The CLI's decimal UI parser is not an exact-money boundary: application code resolves decimal strings through the versioned unit catalog, rejects excess precision, and constructs integer atomic amounts before native message serialization.
...
The Solana slice uses native SVM semantics and the classic SPL Token Program for its initial Circle-USDC-aligned local path. Phase 7A proves the canonical local Token and Associated Token Account programs are sufficient for checked two-decimal mint, wallet transfer, redemption custody, and owner-authorized burn. Its ignored harness rejects redirected tool/runtime paths, restricts local files, and signals a validator only when the saved PID, exact command/scoped ledger, and RPC identity agree; [ADR 0010](adr/0010-native-solana-toolchain-and-classic-spl-semantics.md) records the pinned toolchain and authority/evidence contract.

Phase 7B selects Sava Core/RPC 25.8.0 after an executable Java 25 compatibility gate over the exact public-key, account-layout, instruction, legacy-message, external-signature, and Agave 4.1.2 response shapes the slice uses. Sava and its parser remain confined to `adapters/solana-sava`; only provider-neutral prepared/signature/submission/observation records cross the application port. The adapter constructs canonical idempotent ATA creation plus `MintToChecked`, persists the exact cluster, programs, accounts, recent blockhash, last-valid height, message/instruction digests, ordered fee-payer/mint-authority identities, and policy before effects, and treats the first Ed25519 signature as the stable transaction identity. Sava receives verified public signatures only and cannot access the local keypair files. Submission is fenced once; response loss is inquired by that retained signature; finalized status, transaction bytes/instructions, mint/ATA ownership, exact supply/balance deltas, slot, and block time must agree before blockchain finality advances.

Phase 7C reuses that boundary for one internal server-resolved USER_1-to-USER_2 wallet transfer. The adapter verifies both canonical associated token accounts, creates only an absent destination account, and serializes classic-SPL `TransferChecked` for the exact catalog-derived amount. Ordered fee-payer and source-owner Ed25519 signatures bind the same immutable message. V12 generalizes the existing Solana attempt/signature/observation rows in place to distinguish mint from transfer context and retain source owner/account plus source/destination/supply observations. Replay or response loss inquires by the retained transaction signature; finalized transaction bytes, instruction accounts/data, indexed transaction pre/post token balances, and separately observed token-account owner/mint/program/state and end balances must agree with the exact source decrement, exact destination increment, and zero supply delta before blockchain finality advances. No public API field, redemption, burn, product orchestration, custom program, Token-2022 extension, public cluster, or non-blockchain finality is added.

Phase 7D reuses the provider-neutral redemption handler and that same native boundary for an internal USER_1-to-ADMIN redemption-custody transfer followed by a separate ADMIN-owner burn. The caller still submits only the existing exact burn command; trusted configuration resolves the user source, ADMIN destination/owner, fee payer, mint, programs, roles, wallet-registry version, and policy. Custody is one exact `TransferChecked` signed by fee payer and USER_1. V13 locks the correlated completed transfer and requires its retained attempt, signature, finalized transaction/instructions, indexed source/destination balances, unchanged supply, cluster, registry version, policy, and blockchain-finality evidence to agree before atomically consuming that evidence once with the burn identity. Burn is one exact `BurnChecked` signed by fee payer and ADMIN; finalized transaction bytes, ADMIN account ownership, exact ADMIN decrement, and equal supply decrement must agree before blockchain finality advances. A changed unrelated mint-authority setting cannot relabel a retained ADMIN burn. No public API field, payout/accounting parent, product orchestration, custom program, public cluster, or non-blockchain finality is added.
...
### User-held acquisition and redemption resources

Phase 6B exposes separate local-profile resources:

- `POST /v1/usdzelle/acquisitions` and `GET /v1/usdzelle/acquisitions/{workflowId}`; and
- `POST /v1/usdzelle/redemptions` and `GET /v1/usdzelle/redemptions/{workflowId}`.

Creation requires a scoped idempotency key, exact canonical positive USD amount, one configured participant-owned synthetic bank reference, and optional `ETHEREUM`; distinct acquire/redeem authorities and principal-derived participant scope apply. Wallets, ADMIN authority, asset/unit, contract, signer, policy, steps, children, evidence, status, and outcomes cannot be request fields. Acceptance atomically persists the immutable server-resolved context, ordered plan, first eligible step, history, idempotency binding, and one outbox event before returning 202. Exact replay returns that parent and changed payload conflicts.

The acquisition plan is withdrawal, reserve-funding accounting, mint, mint accounting, and reconciliation. The independent redemption plan is custody transfer, custody accounting, payout, payout accounting, burn, and reconciliation. Each delivery advances at most one durable boundary and re-reads authoritative child state; ambiguity retains the stable child identity for inquiry. Parent execution and mint/burn child routing revalidate the immutable accepted bank, wallet, contract, route, and policy versions, so later configuration rotation fails closed instead of redirecting value. Retained chain evidence is re-read against its original canonical block and exact balances/supply before accounting consumption. Participant-facing status contains only workflow identity/kind/status, canonical amount/currency, network, current step, timestamps, and reconciliation conclusion. The authoritative local contract is `/openapi/usdzelle-workflows-v1.yaml`.

## 16. Persistence, transactions, outbox/inbox, and concurrency

The repository uses PostgreSQL, explicit Spring JDBC `JdbcClient`, HikariCP, and thirteen forward-only Flyway migrations; see [ADR 0004](adr/0004-postgresql-jdbc-flyway-atomic-outbox.md), [ADR 0005](adr/0005-postgresql-operation-delivery-worker.md), [ADR 0007](adr/0007-local-ethereum-mint-vertical-slice.md), and [ADR 0009](adr/0009-synthetic-reserve-ledger-and-reconciliation.md). Normalized tables store token operations, transfers/effects, user-held workflow context/steps/history/children/conclusions, delivery, signing, local Ethereum attempts/evidence, shared local Solana mint/transfer/burn attempts/evidence, synthetic bank fixtures/accounts/operations/balance entries, ledger journals/lines, one-time evidence consumption, operational custody/supply positions, and reconciliation results. Exact quantities are bounded integers rather than floating-point or unbounded aggregate JSON.
...
Phase 7B V11 adds the corresponding local Solana attempt boundary without copying Sava types into core state. It retains two ordered public signatures over one immutable legacy message, a submit fence, recent-blockhash lifetime, replacement lineage for any separately authorized later attempt, and bounded observation rows. A partially signed attempt resumes at the missing signer order; a known-signature ambiguity is observed before any new attempt; an expired unobserved signature is unsafe and enters the existing manual-review boundary. Automatic replacement is not authorized in this slice.

Phase 7C V12 extends those same records rather than adding a second chain lifecycle. It binds each row to mint or wallet-transfer context, retains transfer source authority/account and the immutable destination snapshot, and records independently observed source, destination, and supply deltas. The common wallet-transfer aggregate repository and its V6 migration live in `adapters/persistence-postgres`; neither native adapter owns shared business persistence.

Phase 7D V13 extends the same records for burn context and retains the correlation to one exact redemption-custody operation/effect/attempt. Initial burn preparation locks that shared historical correlation row, verifies the latest confirmed custody observation and matching blockchain finality against immutable transfer context, inserts the durable ADMIN burn attempt, and marks the evidence consumed in one transaction. Row locking and stable-burn correlation prevent a second consumer. Response loss and restart continue through the retained native signature; only proven pre-submission expiry may create a sequence-incremented, explicitly parented native replacement under the same burn identity, without releasing or consuming custody evidence again.
...
Phase 6B V9 atomically accepts each workflow with its scoped hashed idempotency binding, immutable exact context, ordered steps, acceptance transition, and outbox row. Database advisory locking resolves concurrent same-key acceptance before replay/conflict classification; optimistic versions and append-only transition/child/evidence tables preserve one-step progression and restart reconstruction. External bank, signing, RPC, and observation work remains outside workflow transactions. A stable child reference is persisted before later delivery evaluates that child, and the existing outbox lease/inbox fences remain the at-least-once delivery boundary. For payout-gated redemption, the later burn is durably bound to the already confirmed custody evidence only after payout accounting makes burn eligible.
...
- The separate `local-solana` signer loads only the four required Phase 7A mode-`0600` keypair files beneath an ignored mode-`0700` runtime root, verifies each private/public pair and configured public identity, and binds fee-payer, mint-authority, USER_1 transfer-authority, or ADMIN burn-authority role to its exact action, network, source, alias, and key version. Key bytes remain inside that signer and are wiped where Java arrays permit; Sava receives only the exact message plus verified public signatures. This is disposable local authority, not a production key provider.
- No default or production profile activates local bank mocks, accounting, signing, or chain execution. Phase 6A's synthetic-bank endpoints and accounting services require `local-demo`, use distinct least-authority permissions, and contain no real customer or account data. Phase 5A requires explicit `local-signer` plus `local-ethereum`; Phases 5C-5D require explicit `local-demo` plus `local-ethereum`; Phases 7B-7D require the mutually exclusive `local-solana` profile. Chain profiles accept only uncredentialed loopback HTTP, hard-code the permitted local network, and have no default contract/program or destination identity sufficient to start value-moving work. Opaque bank references and wallet roles never become on-chain personal data.
...
The default runtime topology remains one Spring Boot process plus a private/local PostgreSQL 17 database. Integration tests start the pinned `postgres:17.10-alpine3.23` image through Testcontainers, run the fourteen ordered Flyway migrations through V14 from an empty schema in the full composition, and verify the operation/transfer/delivery/signing/chain, synthetic bank/accounting, and composed workflow boundaries. The polling lifecycle, local signers, synthetic banks, and accounting services are disabled by default. `local-demo` alone initializes version-fenced synthetic fixtures and exposes the local mock-bank contract; combined with `local-ethereum` it also composes the configured-custody transfer/redemption/burn handler. `local-signer` plus `local-ethereum` composes the separate Ethereum mint handler. The mutually exclusive `local-demo,local-solana` composition routes the existing user-held and settlement-only parents, mint/burn APIs, internal USER_1-to-USER_2 transfer, and USER_1-to-ADMIN redemption custody through the configured local Ed25519 signer and Sava adapter against explicit loopback Agave identities.

Phase 6D adds a separate local operator topology without changing the default. Root Compose uses digest-pinned PostgreSQL 17.10, Foundry/Anvil 1.5.1, and Temurin 25.0.2 images, one internal named network, named database/chain volumes, and loopback-only host ports. A one-shot Foundry service deploys and verifies the existing contract and roles before a minimal non-root packaged control-plane image becomes ready. The combined `local-demo,local-ethereum,local-demo-environment` profiles add two generated file-backed bearer identities and one fixed-scope read-only status projection. The sender can invoke only the existing configured Phase 6B/6C resources; the operator can inspect only aggregate demo evidence. Ordinary stop/start retains both PostgreSQL and Anvil state; Anvil loads and dumps its compressed state through its local RPC. Explicit `reset.sh --yes` destroys only the named Phase 6D state. No public chain, hosted RPC, real bank/reserve fallback, or production signer fallback is configured.
...
- Bouncy Castle 1.85 is pinned centrally and used only by the local signer and Sava Core's runtime path; existing secp256k1 and JDK Ed25519 signer gates cover the composed version. Web3j Core 4.14.0 remains confined to the tested Ethereum adapter.
...
- Solana uses native SVM semantics and classic SPL Token first; Sava Core/RPC 25.8.0 passed the bounded Java 25 evaluation and remains isolated to the Solana adapter; see ADR 0003.
- The local Solana semantic gate plus mint and wallet-transfer adapter use pinned native Agave/SPL semantics, canonical associated accounts, checked exact-quantity instructions, distinct native fee-payer/authority/owner roles, and no extension/custom program; the adapter owns exact conversion and exact blockhash lifetime before signing; see ADR 0010.
...
- Signing canonicalization version 1, distinct EVM-digest/Solana-message commands, non-secret key metadata, durable provider identity, inquiry-before-retry, redacted evidence persistence, and no runtime fallback implement the already-governed signing boundary without a new provider decision. The configured local Solana provider additionally retains only its bound public signature result in ignored mode-restricted runtime state so a crash after durable `SIGNED` state can recover by provider inquiry without signing again.
- The explicitly enabled local signer uses Bouncy Castle 1.85 only for secp256k1 and JDK-native Ed25519, generates session-only keys, and returns stale pending work to manual review; see ADR 0006.
- Versioned design-first OpenAPI 3.1 YAML resources are authoritative for their API boundaries; no runtime documentation generator or UI is added.
...
Production readiness, real bank integration, dynamic wallet provisioning/administration, production reserve accounting, arbitrary institutional routing, automatic compensation/refund/reversal, automatic replacement transactions, production cryptographic custody/provider integration, cloud/CI deployment, bridge design, consumer wallet software, omnibus beneficial-balance ledger, double-entry ledger completeness, broker/workflow topology, vendor selection, reserve income sharing, SBOM/threat hardening, public testnet/mainnet, and compliance/legal certification remain deferred. The Phase 6D and Phase 7F demonstrations are executable only as synthetic local proofs and do not remove those deferrals. Each future executable slice requires a separately authorized active plan; [`TRANSFER_DEMO.md`](TRANSFER_DEMO.md) defines the demonstrated product meanings and remaining boundaries.

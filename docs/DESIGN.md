...
The repository uses PostgreSQL, explicit Spring JDBC `JdbcClient`, HikariCP, and eleven forward-only Flyway migrations; see [ADR 0004](adr/0004-postgresql-jdbc-flyway-atomic-outbox.md), [ADR 0005](adr/0005-postgresql-operation-delivery-worker.md), [ADR 0007](adr/0007-local-ethereum-mint-vertical-slice.md), and [ADR 0009](adr/0009-synthetic-reserve-ledger-and-reconciliation.md). Normalized tables store token operations, transfers/effects, user-held workflow context/steps/history/children/conclusions, delivery, signing, local Ethereum and Solana mint attempts/evidence, synthetic bank fixtures/accounts/operations/balance entries, ledger journals/lines, one-time evidence consumption, operational custody/supply positions, and reconciliation results. Exact quantities are bounded integers rather than floating-point or unbounded aggregate JSON.
...
- The separate `local-solana` signer loads only the two Phase 7A mode-`0600` keypair files beneath an ignored mode-`0700` runtime root, verifies each private/public pair and configured public identity, and binds fee-payer versus mint-authority role, network, source, alias, and key version. Key bytes remain inside that signer and are wiped where Java arrays permit; Sava receives only the exact message plus verified public signatures. This is disposable local authority, not a production key provider.
- No default or production profile activates local bank mocks, accounting, signing, or chain execution. Phase 6A's synthetic-bank endpoints and accounting services require `local-demo`, use distinct least-authority permissions, and contain no real customer or account data. Phase 5A requires explicit `local-signer` plus `local-ethereum`; Phases 5C-5D require explicit `local-demo` plus `local-ethereum`; Phase 7B requires the mutually exclusive `local-solana` profile. Chain profiles accept only uncredentialed loopback HTTP, hard-code the permitted local network, and have no default contract/program or destination identity sufficient to start value-moving work. Opaque bank references and wallet roles never become on-chain personal data.
...
The default runtime topology remains one Spring Boot process plus a private/local PostgreSQL 17 database. Integration tests start the pinned `postgres:17.10-alpine3.23` image through Testcontainers, run all eleven Flyway migrations from an empty schema in the full composition, and verify the operation/transfer/delivery/signing/chain, synthetic bank/accounting, and composed workflow boundaries. The polling lifecycle, local signers, synthetic banks, and accounting services are disabled by default. `local-demo` alone initializes version-fenced synthetic fixtures and exposes the local mock-bank contract; combined with `local-ethereum` it also composes the configured-custody transfer/redemption/burn handler. `local-signer` plus `local-ethereum` composes the separate Ethereum mint handler. The mutually exclusive `local-solana` profile composes only the existing mint API/queue, the configured local Ed25519 signer, and the Sava adapter against explicit loopback Agave identities.
...
1. native Solana transfer, redemption, burn, and both demonstrations after the completed Phase 7B mint slice; and
2. final architecture, security, recovery, API, and share-readiness review.
...
- The local Solana semantic gate and mint adapter use pinned native Agave/SPL semantics, canonical associated accounts, checked exact-quantity instructions, distinct native fee-payer/authority/owner roles, and no extension/custom program; the adapter owns exact conversion and exact blockhash lifetime before signing; see ADR 0010.
...
Production readiness, real bank integration, dynamic wallet provisioning/administration, production reserve accounting, arbitrary institutional routing, automatic compensation/refund/reversal, automatic replacement transactions, Solana transfer/redemption/burn and product-demonstration parity, production cryptographic custody/provider integration, cloud/CI deployment, bridge design, consumer wallet software, omnibus beneficial-balance ledger, double-entry ledger completeness, broker/workflow topology, vendor selection, reserve income sharing, SBOM/threat hardening, public testnet/mainnet, and compliance/legal certification remain deferred. The Phase 6D demonstrations and Phase 7A/7B local Solana gates are executable only as synthetic local proofs and do not remove those deferrals. Each future executable slice requires a separately authorized active plan; [`TRANSFER_DEMO.md`](TRANSFER_DEMO.md) defines the demonstrated product meanings and remaining boundaries.

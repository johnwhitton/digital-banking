...
The repository uses PostgreSQL, explicit Spring JDBC `JdbcClient`, HikariCP, and twelve forward-only Flyway migrations; see [ADR 0004](adr/0004-postgresql-jdbc-flyway-atomic-outbox.md), [ADR 0005](adr/0005-postgresql-operation-delivery-worker.md), [ADR 0007](adr/0007-local-ethereum-mint-vertical-slice.md), and [ADR 0009](adr/0009-synthetic-reserve-ledger-and-reconciliation.md). Normalized tables store token operations, transfers/effects, user-held workflow context/steps/history/children/conclusions, delivery, signing, local Ethereum attempts/evidence, shared local Solana mint-or-transfer attempts/evidence, synthetic bank fixtures/accounts/operations/balance entries, ledger journals/lines, one-time evidence consumption, operational custody/supply positions, and reconciliation results. Exact quantities are bounded integers rather than floating-point or unbounded aggregate JSON.
...
The default runtime topology remains one Spring Boot process plus a private/local PostgreSQL 17 database. Integration tests start the pinned `postgres:17.10-alpine3.23` image through Testcontainers, run all twelve Flyway migrations from an empty schema in the full composition, and verify the operation/transfer/delivery/signing/chain, synthetic bank/accounting, and composed workflow boundaries. The polling lifecycle, local signers, synthetic banks, and accounting services are disabled by default. `local-demo` alone initializes version-fenced synthetic fixtures and exposes the local mock-bank contract; combined with `local-ethereum` it also composes the configured-custody transfer/redemption/burn handler. `local-signer` plus `local-ethereum` composes the separate Ethereum mint handler. The mutually exclusive `local-solana` profile composes the existing mint API plus one server-owned internal USER_1-to-USER_2 wallet-transfer route through the configured local Ed25519 signer and Sava adapter against explicit loopback Agave identities.
...
1. native Solana redemption custody and burn after the completed Phase 7C wallet-transfer slice;
2. Solana product-path orchestration, followed by both demonstrations; and
...
Production readiness, real bank integration, dynamic wallet provisioning/administration, production reserve accounting, arbitrary institutional routing, automatic compensation/refund/reversal, automatic replacement transactions, Solana redemption/burn, Solana product orchestration, and Solana demonstration parity, production cryptographic custody/provider integration, cloud/CI deployment, bridge design, consumer wallet software, omnibus beneficial-balance ledger, double-entry ledger completeness, broker/workflow topology, vendor selection, reserve income sharing, SBOM/threat hardening, public testnet/mainnet, and compliance/legal certification remain deferred. The Phase 6D demonstrations and Phase 7A-7C local Solana gates are executable only as synthetic local proofs and do not remove those deferrals. Each future executable slice requires a separately authorized active plan; [`TRANSFER_DEMO.md`](TRANSFER_DEMO.md) defines the demonstrated product meanings and remaining boundaries.

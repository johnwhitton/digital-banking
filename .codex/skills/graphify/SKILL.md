...
## Digital Banking repository guardrails

For this repository, these rules override generic instructions later in this skill:

- Use the already installed, reviewed `graphifyy` 0.8.47 tool. Never install, upgrade, downgrade, or replace Graphify automatically; stop and report a missing or different executable.
- Use `.graphifyignore` and do not index `docs/reference/*.pdf`. The Markdown architecture, ADRs, plans, Java/Maven source, tests, and AI workflow files are the approved corpus.
- Graphify 0.8.47 deliberately re-adds `graphify-out/memory/` after ignore processing. Remove every detected `graphify-out/` path before extraction; do not feed saved queries or other generated output back into this repository graph.
- Never select an API/backend from environment-variable presence and never send repository content to an external model or hosted index. Semantic extraction may use only subagents inside the current approved Codex session.
- Do not run `graphify add`, `watch`, `global`, `mcp`, database export/push, or Git-hook installation commands for repository work unless a later user request explicitly authorizes them.
- Keep `graphify-out/GRAPH_REPORT.md`, `graphify-out/graph.json`, and `graphify-out/manifest.json` only after reviewing them for portability and sensitive content. Treat caches, costs, query memory, reflections, interpreter/root files, and HTML as local generated state.
- Graph output is a navigation aid. Direct source, `AGENTS.md`, `SECURITY.md`, accepted ADRs, tests, and recorded validation remain authoritative.

...
request='{"amount":"100","currency":"USD","sourceBankAccountReference":"synthetic-bank:USER_1_BANK_ACCOUNT","destinationBankAccountReference":"synthetic-bank:USER_2_BANK_ACCOUNT","settlementNetwork":"SOLANA"}'
key=phase7f-settlement-$run_id
...
accepted=
demo_wait_parent "/v1/transfers/$transfer_id" "$parent" 900
...
jq -n --arg runId "$run_id" --arg transferId "$transfer_id" \
    '{demo:"B-settlement-only",runId:$runId,amount:"USD 100.00",atomicUnits:"10000",transferId:$transferId,effects:{withdrawal:1,mint:1,userTransfer:1,custodyTransfer:1,payout:1,burn:1},bank:{senderBefore:"10000",senderAfter:"0",recipientBefore:"0",recipientAfter:"10000"},final:{userOne:"0",userTwo:"0",admin:"0",supply:"0"},result:"RECONCILED",replay:"SAME_RESOURCE_NO_NEW_EFFECT"}' \
    > "$work/summary.json"
chmod 600 "$work"/*.json
printf 'Demo B passed: settlement %s, exact six effects, USD cents 10000->0/0->10000, zero final supply/balances, RECONCILED. Summary: %s\n' \
